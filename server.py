  ...
  <div class="glass-card">
    ...
    <div style="margin-top: 25px; border-top: 1px solid rgba(255,255,255,0.1); padding-top: 15px; text-align: left;">
        ...
        {% if files %}
            ...
            {% for name in files %}
                <li style="background: rgba(0,0,0,0.2); padding: 12px; margin-bottom: 8px; border-radius: 8px; display: flex; justify-content: space-between; align-items: center; font-size: 13px;">
                    <span style="overflow: hidden; text-overflow: ellipsis; white-space: nowrap; max-width: 65%;">{{ name }}</span> 
                    <a href="/download/{{ name }}" style="background: #3a7bd5; color: white; padding: 6px 12px; border-radius: 6px; text-decoration: none; font-weight: bold;">GET</a>
                </li>
            {% endfor %}
            </ul>
        {% else %}
            <p style="color: #78909c; font-size: 13px; font-style: italic; text-align: center; margin: 0;">No files shared yet.</p>
        {% endif %}
    </div>
...
def index():
    return render_template_string(HTML_TEMPLATE, files=shared_files.keys())
...
def download(filename):
    if filename in shared_files:
        return send_file(shared_files[filename], as_attachment=True)
